...
//        }
        ...
        adminFileService.removeByBatchId(Db.queryStr("select batch_id from `task` where task_id = ?",taskId));
    ...
    public R restore(Integer taskId){
        ...
        int update;
        if(count>0){
            update = Db.update("update task set ishidden = 0,hidden_time = null where task_id = ?", taskId);
        }else {
            update = Db.update("update task set ishidden = 0,class_id = null,hidden_time = null where task_id = ?", taskId);
        }
