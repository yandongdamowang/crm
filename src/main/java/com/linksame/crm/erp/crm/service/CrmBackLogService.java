...
public class CrmBackLogService {
    ...
    public R todayCustomer(BasePageRequest basePageRequest){
        ...
        StringBuffer stringBuffer = new StringBuffer("select customer_id from crm_customer as a where ");
        if (type == 1){
            stringBuffer.append(" a.customer_id not in (IFNULL((select GROUP_CONCAT(types_id) from admin_record where types = 'crm_customer' and to_days(create_time) = to_days(now())),0)) and to_days(a.next_time) = to_days(now())");
        }else if (type == 2){
            stringBuffer.append(" a.customer_id not in (IFNULL((select GROUP_CONCAT(types_id) from admin_record where types = 'crm_customer' and to_days(create_time) >= to_days(a.next_time)),0)) and to_days(a.next_time) < to_days(now())");
        }else if (type == 3){
            stringBuffer.append(" a.customer_id = any(select types_id from admin_record where types = 'crm_customer' and to_days(create_time) = to_days(now())) and to_days(a.next_time) = to_days(now())");
        }else {
    ...
    public R followLeads(BasePageRequest basePageRequest){
        ...
        Integer type = jsonObject.getInteger("type");
        Integer isSub = jsonObject.getInteger("isSub");
        StringBuffer stringBuffer = new StringBuffer("select leads_id from crm_leads as a where");
        if (type == 1){
    ...
    public R followCustomer(BasePageRequest basePageRequest){
        ...
        Integer type = jsonObject.getInteger("type");
        Integer isSub = jsonObject.getInteger("isSub");
        StringBuilder stringBuffer = new StringBuilder("select customer_id from crm_customer as a where");
        if (type == 1){
    ...
    public R checkReceivables(BasePageRequest basePageRequest){
        ...
        Integer type = jsonObject.getInteger("type");
        Integer isSub = jsonObject.getInteger("isSub");
        StringBuilder stringBuffer = new StringBuilder("select receivables_id from crm_receivables as a inner join admin_examine_record as b on a.examine_record_id = b.record_id left join admin_examine_log as c on b.record_id = c.record_id where ifnull(b.examine_step_id, 1) = ifnull(c.examine_step_id, 1) and");
        if (type == 1) {
            stringBuffer.append(" a.check_status in (0,3) and c.examine_status in (0,3)");
        } else if (type == 2) {
            stringBuffer.append(" a.check_status in (1,2)");
    ...
    public R remindReceivables(BasePageRequest basePageRequest){
        ...
        Integer type = jsonObject.getInteger("type");
        Integer isSub = jsonObject.getInteger("isSub");
        StringBuffer stringBuffer = new StringBuffer("from crm_receivables_plan as a inner join crm_customer as b on a.customer_id = b.customer_id inner join crm_contract as c on a.contract_id = c.contract_id where");
        if (type == 1){
    ...
    public R endContract(BasePageRequest basePageRequest){
        ...
        Integer type = jsonObject.getInteger("type");
        Integer isSub = jsonObject.getInteger("isSub");
        AdminConfig adminConfig = AdminConfig.dao.findFirst("select * from admin_config where name = 'expiringContractDays'");
        StringBuffer stringBuffer = new StringBuffer("select contract_id from crm_contract as a where a.check_status = 1 ");
        if (type == 1){
    ...
    public R putInPoolRemind(BasePageRequest basePageRequest){
        JSONObject jsonObject = basePageRequest.getJsonObject();
        Integer isSub = jsonObject.getInteger("isSub");
        Integer poolSetting = Db.queryInt("select status from admin_config where name = 'customerPoolSetting' limit 1");
        String dealDays = Db.queryStr("select value from admin_config where name = 'customerPoolSettingDealDays' limit 1");
        String followUpDays = Db.queryStr("select value from admin_config where name = 'customerPoolSettingFollowupDays' limit 1");
        AdminConfig remindConfig = AdminConfig.dao.findFirst("select * from admin_config where name = 'putInPoolRemindDays'");
        StringBuffer stringBuffer = new StringBuffer("select customer_id from crm_customer as a");
