...
import com.linksame.crm.erp.admin.entity.AdminField;
import com.linksame.crm.erp.admin.entity.AdminFieldSort;
import com.linksame.crm.erp.admin.entity.AdminFieldStyle;
import com.linksame.crm.erp.admin.entity.AdminFieldv;
import com.linksame.crm.erp.crm.common.CrmEnum;
import com.linksame.crm.erp.crm.common.FormTypeEnum;
import com.linksame.crm.utils.*;
import com.linksame.crm.utils.BaseUtil;
import com.linksame.crm.utils.FieldUtil;
...
public class AdminFieldService {
    ...
    public List<Record> queryUpdateField(Integer label,Record record){
        ...
        recordList.forEach(field ->{
            if (field.getInt("type") == FormTypeEnum.FILE.getType()){
                field.set("value",Db.find("select file_id, name, size, create_user_id, create_time, file_path, file_type from admin_file where batch_id = ?",StrUtil.isNotEmpty(record.get(field.getStr("field_name")))?record.get(field.getStr("field_name")):""));
            }else if (field.getInt("type") == FormTypeEnum.USER.getType()){
                field.set("value",Db.find("select user_id,realname from admin_user where find_in_set(user_id,ifnull(?,0))",record.getStr(field.getStr("field_name"))));
            }else if (field.getInt("type") == FormTypeEnum.STRUCTURE.getType()){
                field.set("value",Db.find("select dept_id,name from admin_dept where find_in_set(dept_id,ifnull(?,0))",record.getStr(field.getStr("field_name"))));
            }else {
    ...
    public List<Record> queryInformation(CrmEnum crmEnum,Record record,List<String> fieldKey){
        ...
        fieldTypeMap.forEach((type,list)->{
            if (type == 1){
                ...
                list.forEach(field->{
                    if (field.getInt("type") == FormTypeEnum.FILE.getType()){
                        field.set("value",Db.find("select file_id, name, size, create_time, file_path, file_type, batch_id from admin_file where batch_id = ?",StrUtil.isNotEmpty(record.get(field.getStr("field_name")))?record.get(field.getStr("field_name")):""));
                    }else if (field.getInt("type") == FormTypeEnum.USER.getType()){
                        field.set("value",Db.find("select user_id,realname from admin_user where find_in_set(user_id,ifnull(?,0))",record.getStr(field.getStr("field_name"))));
                    }else if (field.getInt("type") == FormTypeEnum.STRUCTURE.getType()){
                        field.set("value",Db.find("select dept_id,name from admin_dept where find_in_set(dept_id,ifnull(?,0))",record.getStr(field.getStr("field_name"))));
    ...
    public R save(JSONObject jsonObject) {
        ...
        if (categoryId != null && Db.queryInt("select ifnull(is_sys,0) from oa_examine_category where category_id = ?", categoryId) == 1) {
        ...
        List<AdminField> fieldSorts = AdminField.dao.find("select name from admin_field where label = ?", label);
        ...
        for (int i = 0; i < adminFields.size(); i++) {
            ...
            if (entity.getFieldId() != null) {
                ...
                if (entity.getFieldType() == 0){
                    ...
                    Db.update("update admin_field_sort set name = ? where field_id = ?",entity.getName(),entity.getFieldId());
                }
            } else {
                entity.save();
            }
    ...
    public boolean save(JSONArray array, String batchId) {
        ...
        Db.deleteById("admin_fieldv", "batch_id", batchId);
        ...
        recordList.forEach(record -> {
            if (record.getInt("type") == 10) {
                if (StrUtil.isNotEmpty(record.getStr("value"))) {
                    List<Record> userList = Db.find("select user_id,realname from admin_user where user_id in (" + record.getStr("value") + ")");
                ...
                if (StrUtil.isNotEmpty(record.getStr("value"))) {
                    List<Record> deptList = Db.find("select dept_id,name from admin_dept where dept_id in (" + record.getStr("value") + ")");
    ...
    public List<AdminFieldStyle> queryFieldStyle(String type) {
        ...
        return AdminFieldStyle.dao.find("select * from admin_field_style where type = ? and user_id = ?", type, userId);
    }
    ...
    public List<Record> queryListHead(AdminFieldSort adminFieldSort) {
        ...
        Long userId = BaseUtil.getUser().getUserId();
        Integer number = Db.queryInt("select count(*) from admin_field_sort where user_id = ? and label = ?", userId, adminFieldSort.getLabel());
        if (0 == number) {
    ...
    public R queryFieldConfig(AdminFieldSort adminFieldSort) {
        ...
        for (Record record : fieldList) {
            ...
            Integer number = Db.queryInt("select count(*) as number from admin_field_sort where user_id = ? and label = ? and field_name = ?", userId, adminFieldSort.getLabel(), fieldName);
            if (number.equals(0)) {
    ...
    public void transferFieldList(List<Record> recordList, Integer isDetail){
        recordList.forEach(record -> {
            ...
            if(isDetail == 2){
                ...
                if(FormTypeEnum.USER.getType() == dataType){
                    if(StrUtil.isNotEmpty(record.getStr("value"))){
                        record.set("value",Db.queryStr("select group_concat(realname) from `admin_user` where user_id in ("+record.getStr("value")+")"));
                    }
                }else if (FormTypeEnum.STRUCTURE.getType() == dataType) {
                    if(StrUtil.isNotEmpty(record.getStr("value"))){
                        record.set("value",Db.queryStr("select group_concat(name) from `admin_dept` where dept_id in ("+record.getStr("value")+")"));
                    }
                }
            }
            if(dataType == FormTypeEnum.FILE.getType()){
                if(StrUtil.isNotEmpty(record.getStr("value"))){
                    record.set("value",Db.find("select file_id, name, size, create_user_id, create_time, file_path, file_type from admin_file where batch_id = ?",record.getStr("value")));
                }
            }
        });
    }
