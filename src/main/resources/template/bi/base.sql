#namespace("bi.base")
  #sql ("queryCrmBusinessStatistics")
    ...
    scb.status_id,IFNULL((SELECT sum(money)  from crm_business WHERE scb.status_id = status_id),0)as  total_price,
    scbs.`name`
  ...
  #sql ("queryByUserIdOrYear")
      select co.* from contractview as co LEFT JOIN admin_user as sau on co.owner_user_id = sau.user_id
      where 1 = 1
      #if(year)
  ...
  #sql ("queryProductSell")
      ...
      scc.contract_id,
      sccu.customer_id
      FROM crm_product as scp
      LEFT JOIN crm_contract_product as sccp on sccp.product_id = scp.product_id
      LEFT JOIN crm_contract as scc on scc.contract_id = sccp.contract_id
      LEFT JOIN admin_user as sau on sau.user_id = scc.owner_user_id
      LEFT JOIN crm_customer as sccu on sccu.customer_id = scc.customer_id
		  LEFT JOIN crm_product_category as scpc on scpc.category_id = scp.category_id
   ...
   #end
  #sql("salesTrend")
    select #para(beginTime) as type,IFNULL(SUM(money),0) as contractMoneys, (SELECT IFNULL(SUM(money),0) FROM crm_receivables WHERE DATE_FORMAT( return_time,#para(sqlDateFormat) ) = #para(beginTime)  and check_status = 1 AND owner_user_id in (#for(x:userIds) #(for.index == 0 ? "" : ",") #para(x) #end)) as receivablesMoneys FROM crm_contract as ccco where DATE_FORMAT(ccco.order_date,#para(sqlDateFormat))=#para(beginTime) and ccco.check_status = 1 AND owner_user_id in (#for(x:userIds) #(for.index == 0 ? "" : ",") #para(x) #end)
  #end
