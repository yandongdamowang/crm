#namespace("oa.examine")
  #sql("myInitiate")
    ...
    from oa_examine a left join oa_examine_record b on a.examine_id = b.examine_id left join oa_examine_category c on a.category_id = c.category_id
  ...
  #end
  #sql("myOaExamine")
    select a.*,b.examine_status,b.record_id as examine_record_id,b.examine_step_id,c.category_id,c.title as categoryTitle  from oa_examine a  left join  oa_examine_record b on a.examine_id = b.examine_id left join oa_examine_category c on a.category_id = c.category_id left join oa_examine_log d on d.record_id = b.record_id
    where 1 = 1
      #if(categoryId!=null&&categoryId!="")
      and a.category_id = #para(categoryId)
      #end
  ...
  #end
  #sql("examineCheck")
      select * from oa_examine_check where  is_checked = 1
        #if(userIds)
  ...
  SELECT * FROM oa_examine_step
  WHERE category_id = ? and step_num =  (SELECT step_num FROM oa_examine_step where step_id = ?) + 1
  #end
  #sql("queryNowadayExamineLogByRecordIdAndStepId")
    select * from oa_examine_log where record_id = ? and examine_step_id = ? and examine_user = ? and is_recheck != 1
  #end
  #sql("queryExamineLogByRecordId")
    select soel.* ,
           (SELECT realname from admin_user WHERE user_id = soel.create_user) createUserName,
           (SELECT realname from admin_user WHERE user_id = soel.examine_user) examineUserName
    from oa_examine_log as soel where soel.record_id = #para(recordId) and examine_time is not null order by examine_time desc
  #end
  #sql("queryExamineRecordById")
    SELECT saer.* ,sau.img,sau.realname
    from oa_examine_record as  saer
  ...
  #sql("queryUserByUserId")
    ...
    LEFT JOIN admin_user as saud on saud.user_id = sau.parent_id
    WHERE sau.user_id = ?
  #end
  #sql("queryUserByUserIdAndStatus")
    SELECT DISTINCT user_id, realname , 0 as examine_status,img from admin_user
    WHERE user_id = ?
  #end
  #sql("queryUserByRecordId")
    select sael.examine_time  as examineTime , sael.examine_status,sau.realname,sau.user_id,sau.img
    from oa_examine_log as sael LEFT JOIN admin_user as sau on sau.user_id = sael.examine_user
  ...
  #end
  #sql("queryExamineLog")
    select * from oa_examine_log where record_id = #para(recordId)  and examine_user = #para(examineUser)
      #if(stepId)
  ...
  #sql("queryExamineTypeByRecordId")
    select examine_type from oa_examine_category where category_id = (select oa_examine.category_id from oa_examine where examine_id = (select  oa_examine_record.examine_id from oa_examine_record where record_id = ?))
  #end
  #sql("queryExamineLogByRecordIdByStep")
    ...
    from oa_examine_log as sael LEFT JOIN admin_user as sau on sau.user_id = sael.examine_user LEFT JOIN oa_examine_step as ases on ases.step_id = sael.examine_step_id
  ...
  #sql("queryRecordAndId")
    ...
    from oa_examine_record  as aser LEFT JOIN admin_user as sau on sau.user_id = aser.create_user
    where record_id = ?
  #end
  #sql("queryUserByUserIdAnd")
    SELECT DISTINCT user_id, realname ,img, 0 as examine_status from admin_user
    WHERE user_id = ?
  #end
  #sql("queryRecordByUserIdAndStatus")
    SELECT DISTINCT user_id, realname  ,img, 5 as examine_status, #para(examineTime) as examineTime from admin_user
  ...
  #sql("queryCountByStepId")
    SELECT  DISTINCT ((SELECT COUNT(log_id) FROM oa_examine_log WHERE record_id = #para(recordId) and examine_step_id = #para(stepId))
  - (SELECT COUNT(log_id) FROM oa_examine_log WHERE record_id = #para(recordId) and examine_step_id = #para(stepId) and examine_status = 2 )) as toCount
    FROM admin_examine_log
  #end
  #sql("queryExamineRelation")
    select a.*,b.examine_status,b.record_id as examine_record_id,b.examine_step_id ,c.category_id,c.title as categoryTitle from oa_examine_relation h left join oa_examine a on h.examine_id = a.examine_id left join oa_examine_record b on a.examine_id = b.examine_id left join oa_examine_category c on a.category_id = c.category_id where 1 = 2
    #if(businessIds)
   ...
   #end
   #sql("queryTravel")
    select travel_id as travelId, examine_id as examineId, start_address as startAddress, start_time as startTime, end_address as endAddress, end_time as endTime, traffic, stay, diet, other, money, vehicle, trip, duration, description, batch_id as batchId from oa_examine_travel where examine_id = ?
   #end
   #sql("queryAllExamineCategoryList")
      select category_id,title,type from oa_examine_category where
   ...
   #sql("queryExamineById")
      select a.*,b.title as category,b.type,c.examine_status  from oa_examine as a left join oa_examine_category b on a.category_id = b.category_id left join `oa_examine_record` c on a.examine_id = c.examine_id where a.examine_id = ?
   #end
  #sql("queryFieldByBatchId")
    select a.field_id,a.field_name,a.name,a.type,a.input_tips,a.options,a.is_unique,a.is_null,b.value,a.field_type from admin_field a left join `admin_fieldv` b on a.field_id = b.field_id where a.examine_category_id = ? and b.batch_id = ? order by sorting
  #end
  #sql("queryExamineCategoryByType")
    SELECT * FROM oa_examine_category WHERE type= ? limit 0,1
  #end
    #sql("queryExamineUserByExamineStepId")
    SELECT * FROM `oa_examine_log`
  ...
  #sql("queryExamineStepByLastExamineIdOrderByStepId")
        SELECT * FROM oa_examine_step WHERE category_id = ?
        and step_num =  (SELECT step_num FROM oa_examine_step where step_id = ?) - 1
   #end
#end
