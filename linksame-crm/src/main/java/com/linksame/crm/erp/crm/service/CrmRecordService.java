...
public class CrmRecordService<T extends Model>{
    ...
    public void updateRecord(JSONArray jsonArray, String batchId){
        ...
        List<AdminFieldv> oldFieldList = new AdminFieldv().dao().find("select * from admin_fieldv where batch_id = ?", batchId);
            ...
            newEntries.forEach(y -> {
                ...
                if(x.getKey().equals(y.getKey()) && ! oldValue.equals(newValue)){
                    if("owner_user_id".equals(x.getKey())){
                        ...
                        oldValue = Db.queryStr("select realname from `admin_user` where user_id = ?", oldValue);
                    }else if("customer_id".equals(x.getKey())){
                        newValue = Db.queryStr("select customer_name from `crm_customer` where customer_id = ?", newValue);
                        oldValue = Db.queryStr("select customer_name from `crm_customer` where customer_id = ?", oldValue);
    ...
    public void addConversionRecord(Integer actionId, CrmEnum crmEnum, Long userId){
        String name = Db.queryStr("select realname from admin_user where user_id = ?", userId);
        CrmActionRecord crmActionRecord = new CrmActionRecord();
        crmActionRecord.setCreateUserId(BaseUtil.getUser().getUserId());
        crmActionRecord.setCreateTime(new Date());
        crmActionRecord.setTypes(crmEnum.getType() + "");
    ...
    public R deleteFollowRecord(Integer recordId){
        adminFileService.removeByBatchId(Db.queryStr("select batch_id from `admin_record` where record_id = ?",recordId));
    ...
    public R queryRecordOptions(){
        List<String> list = Db.query("select value from admin_config where name = 'followRecordOption'");
    ...
    public R setRecordOptions(List<String> list){
        Db.delete("delete from admin_config where name = 'followRecordOption'");
