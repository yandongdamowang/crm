...
import com.linksame.crm.erp.work.entity.WorkTaskLabel;
...
public class LabelService{
    ...
    public R deleteLabel(String labelId){
        Integer count = Db.queryInt("select count(*) from task where label_id like concat('%,',?,',%');", labelId);
        if(count>0){
            return R.error("使用中的标签不能删除");
        }
        Db.delete("delete from work_task_label where label_id = ?",labelId);
        return R.ok();
    }
    ...
    public R getLabelListByOwn(){
        ...
        if(AuthUtil.isWorkAdmin()){
            ...
            resultList = new WorkTaskLabel().find("select * from `work_task_label` where label_id in (?)", String.join(",", collect));
        }
