...
public class CrmContractService {
    ...
    public R deleteByIds(String contractIds) {
        ...
        for (String id : idsArr) {
            ...
            if (contract != null) {
                Db.delete("delete FROM admin_fieldv where batch_id = ?", contract.getBatchId());
            }
    ...
    public R saveAndUpdate(JSONObject jsonObject) {
        ...
        if (jsonArray != null) {
            ...
            if (crmContract.getBusinessId() != null) {
                Db.delete("delete from crm_business_product where business_id = ?", crmContract.getBusinessId());
            }
    ...
    public List<CrmContract> queryList(CrmContract crmContract) {
        StringBuilder sql = new StringBuilder("select * from crm_contract where 1 = 1 ");
    ...
    public R updateOwnerUserId(CrmCustomer crmCustomer) {
        ...
        String contractIds = Db.queryStr("select GROUP_CONCAT(contract_id) from crm_contract where customer_id in (" + crmCustomer.getCustomerIds() + ")");
    ...
    public R addMember(CrmContract crmContract) {
        ...
        for (String id : contractIdsArr) {
            ...
            if (1 == crmContract.getPower()) {
                ...
                Db.update("update crm_contract set ro_user_id = ? where contract_id = ?", roUserId, Integer.valueOf(id));
            }
            if (2 == crmContract.getPower()) {
                stringBuilder.setLength(0);
                ...
                Db.update("update crm_contract set rw_user_id = ? where contract_id = ?", rwUserId, Integer.valueOf(id));
            }
        }
        return R.ok();
    }
