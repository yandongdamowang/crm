...
import com.linksame.crm.erp.crm.entity.CrmActionRecord;
...
public class CrmLeadsService {
    ...
    public R translate(String leadsIds) {
        ...
        for (String leadsId : leadsIdsArr) {
            ...
            List<AdminField> customerFields = AdminField.dao.find("select field_id,name,field_name,field_type,is_null,is_unique from admin_field where label = '2'");
...
//                        }
            ...
            Db.update("update crm_leads set is_transform = 1,update_time = ?,customer_id = ? where leads_id = ?",
                    DateUtil.date(), crmCustomer.getCustomerId(), Integer.valueOf(leadsId));
            List<AdminRecord> adminRecordList = AdminRecord.dao.find("select * from admin_record where types = 'crm_leads' and types_id = ?",Integer.valueOf(leadsId));
            List<CrmActionRecord> crmActionRecordList = CrmActionRecord.dao.find("select * from `crm_action_record` where action_id = ? and types = 1", Integer.valueOf(leadsId));
            ...
            if (adminRecordList.size() != 0){
                adminRecordList.forEach(adminRecord -> {
                    List<AdminFile> leadsRecordFiles = AdminFile.dao.find("select file_id, name, size, create_user_id, create_time, file_path, file_type from admin_file where batch_id = ?",adminRecord.getBatchId());
            ...
            List<AdminFile> fileList = AdminFile.dao.find("select file_id, name, size, create_user_id, create_time, file_path, file_type from admin_file where batch_id = ?",crmLeads.getStr("batch_id"));
            if (fileList.size() != 0){
