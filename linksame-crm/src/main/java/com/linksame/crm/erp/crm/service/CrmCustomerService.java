package com.linksame.crm.erp.crm.service;
...
import com.linksame.crm.erp.crm.common.CrmParamValid;
...
public class CrmCustomerService{
    ...
    public Record queryById(Integer customerId){
        ...
        List<Record> recordList = Db.find("select name,value from `admin_fieldv` where batch_id = ?", record.getStr("batch_id"));
    ...
    public R deleteByIds(String customerIds){
        ...
        return Db.tx(() -> {
            ...
            Db.batch("delete from admin_fieldv where batch_id = ?", "batch_id", batchIdList, 100);
            return true;
        }) ? R.ok() : R.error();
    }
    ...
    public R addMember(CrmCustomer crmCustomer){
        ...
        for(String id : customerIdsArr){
            ...
            if(1 == crmCustomer.getPower()){
                ...
                Db.update("update crm_customer set ro_user_id = ? where customer_id = ?", roUserId, Integer.valueOf(id));
            }
            if(2 == crmCustomer.getPower()){
                stringBuffer.setLength(0);
                ...
                Db.update("update crm_customer set rw_user_id = ? where customer_id = ?", rwUserId, Integer.valueOf(id));
            }
        }
        return R.ok();
    }
    ...
    public String getContractIdsByCustomerIds(String customerIds){
        ...
        for(String id : customerIdsArr){
            List<Record> recordList = Db.find("select contract_id from crm_contract where customer_id = ?", id);
            if(recordList != null){
                for(Record record : recordList){
    ...
    public String getBusinessIdsByCustomerIds(String customerIds){
        ...
        StringBuffer stringBuffer = new StringBuffer();
        for(String id : customerIdsArr){
            List<Record> recordList = Db.find("select business_id from crm_business where customer_id = ?", id);
            if(recordList != null){
                for(Record record : recordList){
    ...
    public R updateRulesSetting(Integer dealDay, Integer followupDay, Integer type, Integer remindDay, Integer remindConfig){
        Db.update("update admin_config set value = ? where name = 'customerPoolSettingDealDays'", dealDay);
        Db.update("update admin_config set value = ? where name = 'customerPoolSettingFollowupDays'", followupDay);
        Db.update("update admin_config set status = ? where name = 'customerPoolSetting'", type);
        if(remindDay > 0){
            Db.update("update admin_config set status = ?,value = ? where name = 'putInPoolRemindDays'", remindConfig, remindDay);
        }else{
            Db.update("update admin_config set status = 0 where name = 'putInPoolRemindDays'");
        }
        return R.ok();
    }
    ...
    public R getRulesSetting(){
        String dealDay = Db.queryStr("select value from admin_config where name = 'customerPoolSettingDealDays'");
        String followupDay = Db.queryStr("select value from admin_config where name = 'customerPoolSettingFollowupDays'");
        Integer type = Db.queryInt("select status from admin_config where name = 'customerPoolSetting'");
        AdminConfig remindConfig = AdminConfig.dao.findFirst("select * from admin_config where name = 'putInPoolRemindDays'");
        ...
        AdminConfig config = AdminConfig.dao.findFirst("select status,value from admin_config where name = 'expiringContractDays' limit 1");
        if(config == null){
    ...
    public R updateCustomerByIds(String ids){
        StringBuffer sq = new StringBuffer("select count(*) from crm_customer where customer_id in ( ");
        sq.append(ids).append(") and is_lock = 1");
        Integer count = Db.queryInt(sq.toString());
        if(count > 0){
            return R.error("选中的客户有被锁定的，不能放入公海！");
        }
        StringBuffer sql = new StringBuffer("UPDATE crm_customer SET owner_user_id = null,ro_user_id = ',',rw_user_id = ',' where customer_id in (");
        ...
        for(String id : idsArr){
            ...
            crmOwnerRecord.save();
            Db.update("update crm_contacts set owner_user_id = null where customer_id = ?", id);
        }
    ...
    public R getCustomersByIds(String ids, Long userId){
        ...
        for(String id : idsArr){
            ...
            crmOwnerRecord.setCreateTime(DateUtil.date());
            crmOwnerRecord.save();
            Db.update("update crm_contacts set owner_user_id = ? where customer_id = ?", userId, id);
        }
    ...
    public R customerSetting(CrmCustomerSetting customerSetting){
        ...
        Db.tx(() -> {
            Db.delete("delete from `crm_customer_settinguser` where setting_id = ?", customerSetting.getSettingId());
    ...
    public R queryEditCustomerSetting(Integer settingId){
        ...
        crmCustomerSetting.put("userIds", Db.query("SELECT user_id FROM crm_customer_settinguser WHERE type='1' and setting_id =?", settingId));
        crmCustomerSetting.put("deptIds", Db.query("SELECT dept_id FROM crm_customer_settinguser WHERE type='2' and setting_id =?", settingId));
    ...
    public R deleteCustomerSetting(Integer settingId){
        ...
        if(crmCustomerSetting != null){
            ...
            Db.delete("delete from `crm_customer_settinguser` where setting_id = ?", settingId);
        }
        return R.ok();
    }
