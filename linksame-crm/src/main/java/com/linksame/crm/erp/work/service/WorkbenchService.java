...
public class WorkbenchService{
            ...
            if(StrUtil.isNotBlank(task.getStr("label_id"))){
                ...
                for(String lableId : lableIds){
                    if(StrUtil.isNotBlank(lableId)){
                        Record lable = Db.findFirst("select label_id,name as labelName,color from work_task_label where label_id = ?", lableId);
                        labelList.add(lable);
                    }
                }
            }
            task.set("labelList", labelList);
            TaskRelation taskRelation = new TaskRelation().findFirst("select * from `task_relation` where task_id = ?", taskId);
            int relationCount = 0;
    ...
    public R updateTop(JSONObject jsonObject){
        String updateSql = "update `task` set is_top = ?,top_order_num = ? where task_id = ?";
