...
import com.linksame.crm.erp.admin.entity.AdminExamineLog;
...
import com.linksame.crm.erp.crm.entity.CrmContacts;
...
public class OaExamineService{
    ...
    public void transfer(List<Record> recordList){
        recordList.forEach(record -> {
            setRelation(record);
            record.set("createUser", Db.findFirst("select user_id,realname,img from admin_user where user_id = ?", record.getLong("create_user_id")));
    ...
    private void setCountRecord(Record record){
        ...
        Integer count = Db.queryInt("select count(*) as count from oa_examine_travel where examine_id = ?", examineId);
    ...
    public R setOaExamine(JSONObject jsonObject){
        ...
        if(oaExamine.getExamineId() == null){
            ...
            Db.delete("delete from oa_examine_travel where examine_id = ?", oaExamine.getExamineId());
            Db.delete("delete from oa_examine_relation where examine_id = ?", oaExamine.getExamineId());
        ...
        if(recordId == null){
            ...
            //更新审核日志状态
            Db.update("update oa_examine_log set is_recheck = 1 where record_id = ?", recordId);
        }
        ...
        if(examineType == 1){
            ...
            if(stepType == 1){
                checkUserIds = Db.queryInt("select parent_id from admin_user where user_id = ?", BaseUtil.getUser().getUserId()) + "";
            }else if(stepType == 4){
                checkUserIds = Db.queryInt("select parent_id from admin_user where user_id = (select parent_id from admin_user where user_id = ?)", BaseUtil.getUser().getUserId()) + "";
            }else{
    ...
    public R oaExamine(OaExamineLog nowadayExamineLog, Long nextUserId){
        ...
        if(examineCategory.getExamineType() == 1){
            log = Db.findFirst("select log_id,order_id from oa_examine_log where record_id = ? and examine_step_id = ? and examine_user = ? and is_recheck = 0", examineRecord.getRecordId(), examineRecord.getExamineStepId(), auditUserId);
        }else{
            log = Db.findFirst("select log_id,order_id from oa_examine_log where record_id = ? and examine_status = 0 and examine_user = ? and is_recheck = 0", examineRecord.getRecordId(), auditUserId);
        }
        ...
        if(status == 2){
            ...
            //先查询该审批流程的审批步骤的第一步
            OaExamineStep oneExamineStep = OaExamineStep.dao.findFirst("SELECT * FROM oa_examine_step WHERE category_id = ? ORDER BY step_num LIMIT 0,1", examine.getCategoryId());
            //判断审核撤回
            ...
            if(examineCategory.getExamineType() == 1){
                ...
                Integer orderId = Db.queryInt("select order_id from oa_examine_log where record_id = ? and is_recheck = 0 and examine_status !=0 order by order_id desc limit 1 ", recordId);
                if(orderId == null){
                    orderId = 1;
                }
            ...
            if(examineCategory.getExamineType() == 1){
                ...
                if(flag){
                    ...
                    if(nextExamineStep != null){
                        ...
                        //生成审批日志
                        if(stepType == 1){
                            checkUserIds = Db.queryInt("select parent_id from admin_user where user_id = ?", createUserId) + "";
                        }else if(stepType == 4){
            ...
            if(((examineCategory.getExamineType() == 2) || (examineCategory.getExamineType() == 1 && flag)) && StrUtil.isEmpty(checkUserIds)){
                ...
                examineRecord.setExamineStatus(3);
                //把下一步审批人放入操作记录中
                OaActionRecord oaActionRecord = new OaActionRecord().findFirst("select * from `oa_action_record` where type = 5 and action_id = ? and content = '添加了审批' limit 1", examineId);
    ...
    public R queryOaExamineInfo(String id){
        ...
        oaExamineInfo.set("createUser", Db.findFirst("select user_id,realname,img from admin_user where user_id = ?", oaExamineInfo.getLong("create_user_id")));
        String batchId = oaExamineInfo.getStr("batch_id");
        setRelation(oaExamineInfo);
        Record first = Db.findFirst("select * from oa_examine_record where examine_id = ?", id);
        ...
        List<Record> examineTravelList = Db.find("select * from oa_examine_travel where examine_id = ?", id);
    ...
    public R getField(String id, Integer isDetail){
        Record oaExamineInfo = Db.findFirst("select * from oa_examine where examine_id = ?", id);
    ...
    private void setRelation(Record relationRecord){
        List<Record> recordList = Db.find("select * from oa_examine_relation where examine_id = ?", relationRecord.getInt("examine_id"));
    ...
    public R deleteOaExamine(Integer oaExamineId){
        Integer recordId = Db.queryInt("select record_id from oa_examine_record where examine_id  = ? limit 1", oaExamineId);
        Db.delete("delete from `admin_fieldv` where batch_id = (select `oa_examine`.batch_id from `oa_examine` where examine_id = ?)", oaExamineId);
        Db.delete("delete from oa_examine where examine_id = ?", oaExamineId);
        Db.delete("delete from oa_examine_relation where examine_id = ?", oaExamineId);
        Db.delete("delete from oa_examine_travel where examine_id = ?", oaExamineId);
        Db.delete("delete from oa_examine_record where record_id = ?", recordId);
        Db.delete("delete from oa_examine_log where record_id = ?", recordId);
    ...
    public R queryExaminStep(String categoryId){
        Record record = Db.findFirst("select examine_type from oa_examine_category where category_id = ?", categoryId);
        Integer examineType = record.getInt("examine_type");
        if(examineType == 1){
            List<Record> recordList = Db.find("select * from oa_examine_step where category_id = ?", categoryId);
    ...
    public R queryExamineRecordList(Integer recordId){
        ...
        if(oaExamineCategory.getExamineType() == 2){
            ...
            jsonObject.put("examineType", 1);
            //固定审批
            List<Record> steps = Db.find("select * from oa_examine_step where  category_id = ? ORDER BY step_num", oaExamineCategory.getCategoryId());
            //上一审核步骤
