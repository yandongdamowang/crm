...
import com.linksame.crm.erp.crm.entity.CrmProduct;
...
public class CrmProductService {
    ...
    public Record queryById(Integer id) {
        ...
        List<Record> recordList = Db.find("select name,value from `admin_fieldv` where batch_id = ?", crmProduct.getStr("batch_id"));
    ...
    public R deleteByIds(String productIds) {
        ...
        for (String id : ids) {
            ...
            if (product != null) {
                Db.delete("delete FROM admin_fieldv where batch_id = ?",product.getBatchId());
                product.setStatus(3);
                product.update();
            }
        }
    ...
    public R updateStatus(String ids, Integer status) {
        List<Record> recordList = Db.find("select batch_id from crm_product where  product_id in (" + ids + ")");
        ...
        StringBuilder sqlfield = new StringBuilder("update admin_fieldv set value = '" + a + "' where name = '是否上下架' and batch_id in ( ");
