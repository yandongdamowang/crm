...
import com.linksame.crm.erp.admin.entity.CrmBusinessStatus;
import com.linksame.crm.erp.admin.entity.CrmBusinessType;
...
public class AdminBusinessTypeService {
    ...
    public R addBusinessType(CrmBusinessType crmBusinessType, JSONArray crmBusinessStatusList) {
        ...
        if (crmBusinessType.getTypeId() == null) {
            ...
            Integer count = Db.queryInt("select count(*) from crm_business where type_id = ?", crmBusinessType.getTypeId());
            if (count > 0) {
    ...
    public R deleteById(String typeId) {
        Integer count = Db.queryInt("select count(*) from crm_business where type_id = ?", typeId);
        if (count > 0) {
            return R.error("使用中的商机组不可以删除");
        }
        Db.deleteById("crm_business_type", "type_id", typeId);
        Db.deleteById("crm_business_status", "type_id", typeId);
        return R.ok();
    }
