...
import com.linksame.crm.utils.Sort;
...
public class AdminUserService {
    ...
    public R setUser(AdminUser adminUser, String roleIds) {
        ...
        if (adminUser.getUserId() == null) {
            ...
            String username = Db.queryStr("select username from admin_user where user_id = ?", adminUser.getUserId());
            if (!username.equals(adminUser.getUsername())) {
                return R.error("用户名不能修改！");
            }
            bol = adminUser.update();
            Db.delete("delete from admin_user_role where user_id = ?", adminUser.getUserId());
            Db.delete("delete from admin_scene where user_id = ? and is_system = 1", adminUser.getUserId());
        }
    ...
    public List<Record> queryTopUserList(Long userId) {
        List<Record> recordList = Db.find("select user_id,realname,parent_id from admin_user");
    ...
    public List<Integer> queryChileDeptIds(Integer deptId,Integer deepness) {
        List<Integer> list = Db.query("select dept_id from admin_dept where pid = ?", deptId);
    ...
    public List<Long> queryChileUserIds(Long userId,Integer deepness) {
        List<Long> query = Db.query("select user_id from admin_user where parent_id = ?", userId);
        if (deepness > 0) {
    ...
    public List<Long> queryUserIdsByParentId(Long userId) {
        String sql = "select user_id from admin_user where parent_id = ? ";
    ...
    public R setUserStatus(String ids, String status) {
        for (Integer id : TagUtil.toSet(ids)) {
            Db.update("update admin_user set status = ? where user_id = ?", status, id);
            if("0".equals(status)){
    ...
    public List<Long> queryUserByParentUser(Long userId, Integer deepness) {
        ...
        if (deepness > 0) {
            List<Long> records = Db.query("SELECT b.user_id FROM admin_user AS b WHERE b.parent_id = ?", userId);
    ...
    public Record queryByDeptIds(String deptIds,String userIds){
        ...
        List<Record> allDepts = Db.find("select * from admin_dept where dept_id in ( ? )",deptIds);
    ...
    private String getDeptIds(String deptIds, List<Record> allDepts){
        for ( Record dept: allDepts) {
            ...
            if (pid != 0){
                deptIds = getDeptIds(deptIds,  Db.find("select * from admin_dept where dept_id in ( ? )",pid));
            }else {
                if (deptIds == null){
    ...
    public R usernameEdit(Integer id,String username, String password){
        ...
        Integer count = Db.queryInt("select count(*) from admin_user where username = ?", username);
        if (count > 0) {
    ...
    private String getUserIds(String deptIds,String userIds){
        List<Record> allUsers = Db.find("select * from admin_user where dept_id NOT in (?) and user_id in (?)", deptIds, userIds);
        userIds = null;
