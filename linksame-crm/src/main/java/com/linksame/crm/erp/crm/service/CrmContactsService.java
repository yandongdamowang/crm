...
import com.linksame.crm.erp.crm.entity.CrmContactsBusiness;
...
public class CrmContactsService {
    ...
    public Record queryById(Integer contactsId){
        ...
        List<Record> recordList = Db.find("select name,value from `admin_fieldv` where batch_id = ?", crmContacts.getStr("batch_id"));
    ...
    public R relateBusiness(Integer contactsId, String businessIds){
        ...
        Db.delete("delete from crm_contacts_business where contacts_id = ?",contactsId);
    ...
    public R updateOwnerUserId(Integer customerId,Long ownerUserId){
        List<Integer> contactsIdList = Db.query("select contacts_id from crm_contacts where customer_id = ?",customerId);
        ...
        Db.update("update crm_contacts set owner_user_id = " + ownerUserId + " where customer_id = "+customerId);
