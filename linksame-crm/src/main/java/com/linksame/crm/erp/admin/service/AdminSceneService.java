...
import com.linksame.crm.erp.admin.entity.AdminScene;
import com.linksame.crm.erp.admin.entity.AdminSceneDefault;
import com.linksame.crm.erp.crm.common.CrmEnum;
import com.linksame.crm.erp.crm.service.CrmBusinessService;
import com.linksame.crm.utils.BaseUtil;
import com.linksame.crm.utils.FieldUtil;
import com.linksame.crm.utils.ParamsUtil;
...
public class AdminSceneService{
    ...
    public R updateScene(AdminScene adminScene){
        ...
        if(1 == adminScene.getIsDefault()){
            Db.update("update admin_scene_default set scene_id = ? where user_id = ? and type = ?", adminScene.getSceneId(), userId, oldAdminScene.getType());
        }
    ...
    public R setDefaultScene(Integer sceneId){
        ...
        Db.delete("delete from admin_scene_default where user_id = ? and type = ?", userId, oldAdminScene.getType());
        AdminSceneDefault adminSceneDefault = new AdminSceneDefault();
    ...
    public String getSubUserId(Long userId, Integer deepness){
        ...
        if(deepness > 0){
            List<Long> list = Db.query("select user_id from admin_user where parent_id = ?", userId);
    ...
    public R getCrmPageList(BasePageRequest basePageRequest){
        ...
        switch(type){
            ...
            case 0:
                ...
                if(jsonObject.getJSONObject("data").containsKey("create_time")){
                    ...
                    sqlObject.put("sql", "and (SELECT COUNT(*) FROM crm_business_change as b WHERE b.business_id = a.business_id and (b.create_time between '"+date.getString("start")+"' and  '"+date.getString("end")+"'))>0");
        ...
        if(2 == type){
            Integer configType = Db.queryInt("select status from admin_config where name = 'customerPoolSetting'");
            selectSql = 1 == configType ? Db.getSql("admin.scene.getCustomerPageList") : "select *,(select count(*) from crm_business as a where a.customer_id = views.customer_id) as business_count";
        }else if(6 == type){
            selectSql = "select *,IFNULL((select SUM(a.money) from crm_receivables as a where a.check_status = '1' and a.contract_id = views.contract_id),0) as receivedMoney,(IFNUll(money,0) - IFNULL((select SUM(a.money) from crm_receivables as a where a.check_status = '1' and a.contract_id = views.contract_id),0)) as unreceivedMoney";
        }else if(9 == type){
    ...
    public Map<String,AdminField> getAdminFieldMap(Integer type){
        ...
        if(adminFieldMap == null){
            List<AdminField> adminFields = AdminField.dao.find("SELECT field_name,field_type,type FROM admin_field WHERE label=?", type);
    ...
    public void setBusinessStatus(List<Record> list){
        list.forEach(record -> {
            if(record.getInt("is_end") == 0){
                Integer sortNum = Db.queryInt("select order_num from crm_business_status where status_id = ?", record.getInt("status_id"));
                int totalStatsNum = Db.queryInt("select count(*) from crm_business_status where type_id = ?", record.getInt("type_id")) + 1;
                if(sortNum == null){
                    sortNum = 0;
                }
