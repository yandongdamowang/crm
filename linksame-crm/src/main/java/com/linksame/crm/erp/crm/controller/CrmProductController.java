...
public class CrmProductController extends Controller {
    ...
    public void queryById(@Para("productId") Integer productId) {
        Integer number = Db.queryInt("select count(1) from `crm_product` where product_id = ? and status != 3",productId);
        if (number == 0) {
    ...
    public void downloadExcel() {
        ...
        List<String> categoryList = Db.query("select name from crm_product_category");
        try {
