...
import com.linksame.crm.erp.admin.entity.AdminRole;
import com.linksame.crm.erp.admin.entity.AdminRoleMenu;
import com.linksame.crm.erp.admin.entity.AdminUserRole;
...
public class AdminRoleService {
    ...
    public List<Record> getRoleByType(Integer roleType){
        List<Record> recordList = Db.find("select * from admin_role where role_type = ?",roleType);
        String realm;
        switch (roleType){
            case 1:
                realm = "manage";
        ...
        Integer pid = Db.queryInt("select menu_id from admin_menu where parent_id = 0 and realm = ?",realm);
    ...
    public R save(AdminRole adminRole) {
        Integer number = Db.queryInt("select count(*) from admin_role where role_name = ? and role_type = ?", adminRole.getRoleName(), adminRole.getRoleType());
        if (number > 0) {
    ...
    public JSONObject auth(Long userId) {
        ...
        if (roleIds.contains(BaseConstant.SUPER_ADMIN_ROLE_ID)) {
            ...
            if(roleIds.contains(BaseConstant.WORK_ADMIN_ROLE_ID)){
                menuRecords.add(Db.findFirst("select realm,menu_id,parent_id from `admin_menu` where remarks = 'projectSave'"));
            }
        }
        ...
        List<String> moduleName = Db.query("select name from admin_config where name in ('oa','crm','project') and status = 0");
    ...
    public boolean delete(Integer roleId) {
        Record record = Db.findFirst("select count(*) as menuNum from admin_role_menu where role_id = ?", roleId);
    ...
    public boolean deleteWorkRole(Integer roleId) {
        Db.delete(Db.getSql("admin.role.deleteRole"), roleId);
        Db.delete(Db.getSql("admin.role.deleteRoleMenu"), roleId);
        Db.update("update `work_user` set role_id = ? where role_id = ?",BaseConstant.SMALL_WORK_EDIT_ROLE_ID,roleId);
        return true;
    }
    ...
    public void copy(Integer roleId) {
        ...
        if (!ReUtil.contains("^[(]\\d+[)]$", roleName)) {
            adminRoleList = AdminRole.dao.find("select * from admin_role where role_name like '" + pre + "%'");
        } else {
            adminRoleList = AdminRole.dao.find("select * from admin_role where role_name regexp '^[(]\\d+[)]$'");
        }
        ...
        if (adminUserRole != null && adminUserRole.getUserIds() != null) {
            ...
            for (String userId : userIdsArr) {
                for (String roleId : roleIdsArr) {
                    Db.delete("delete from admin_user_role where user_id = ? and role_id = ?", Integer.valueOf(userId), Integer.valueOf(roleId));
    ...
    public R unbindingUser(AdminUserRole adminUserRole) {
        ...
        if(Db.queryInt("select count(1) from `admin_user_role` where user_id = ?",adminUserRole.getUserId())==1){
            return R.error("用户至少需要一个角色");
        }
        return Db.delete("delete from admin_user_role where user_id = ? and role_id = ?", adminUserRole.getUserId(), adminUserRole.getRoleId()) > 0 ? R.ok() : R.error();
    }
    ...
    public R queryProjectRoleList(){
        List<Record> roleList = Db.find("select * from admin_role where role_type in (5,6) and is_hidden = 1");
        roleList.forEach(record -> {
            List<Integer> rules = Db.query("select menu_id from admin_role_menu where role_id = ?",record.getInt("role_id"));
    ...
    public R setWorkRole(JSONObject jsonObject){
        ...
        if(roleId == null){
            ...
            Db.delete("delete from `admin_role_menu` where role_id = ?",roleId);
