#namespace("admin.scene")
    ...
    #sql ("queryCrmPageListByFieldType1")
      #(select) FROM (
        SELECT
          `a`.*,
          (SELECT realname FROM admin_user WHERE user_id=`a`.`create_user_id`) AS `create_user_name`,
          (SELECT realname FROM admin_user WHERE user_id=`a`.`owner_user_id`) AS `owner_user_name`,
          #if(label==3)
        ...
        FROM
        ...
        #elseif(label==4)
          LEFT JOIN `crm_product_category` `d` ON `a`.`category_id` = `d`.`category_id`
        #elseif(label==5)
          LEFT JOIN `crm_customer` `d` ON `a`.`customer_id` = `d`.`customer_id`
          LEFT JOIN `crm_business_type` `e` ON `a`.`type_id` = `e`.`type_id`
          ...
          LEFT JOIN `crm_customer` `d` ON `a`.`customer_id` = `d`.`customer_id`
				...
				#elseif(label==7)
          LEFT JOIN `crm_customer` `d` ON `a`.`customer_id` = `d`.`customer_id`
          LEFT JOIN `crm_contract` `e` ON `a`.`contract_id` = `e`.`contract_id`
          LEFT JOIN `crm_receivables_plan` `f` ON `a`.`plan_id` = `f`.`plan_id`
        #end
        JOIN (
          SELECT
            ...
            #for(query : queryList) #if(query.get("type")==1) #(query.get("sql")) #end #end #if(orderByKey!="create_user_name"&&orderByKey!="owner_user_name") ORDER BY #(orderByKey) #(orderByType) #end #if(!field) #if(page&&limit) LIMIT #(page),#(limit) #end #end
              ...
              left join admin_user d on find_in_set(d.user_id,ifnull(a.value,0))
            #end
            #if(fieldMap.containsKey("dept"))
              left join admin_dept c on find_in_set(c.dept_id,ifnull(a.value,0))
            #end
            GROUP BY b.batch_id
    ...
    #sql ("queryCrmPageListByFieldType2")
      #(select) FROM (
        ...
        LEFT JOIN (
          ...
          FROM
            admin_fieldv AS a
          JOIN admin_field as f on a.field_id=f.field_id and f.label=#para(label)
          #if(fieldMap.containsKey("user"))
            left join admin_user d on find_in_set(d.user_id,ifnull(a.value,0))
          #end
          #if(fieldMap.containsKey("dept"))
    ...
    #end
    #sql ("queryHideScene")
    select scene_id,name,data from admin_scene where type = ? and user_id = ? and is_hide = 1
    #end

    #sql ("querySystemNumber")
    select count(*) from admin_scene where is_system = 1 and type = ? and user_id = ?
    #end

    #sql ("sort")
    update admin_scene set is_hide = 0,sort = ? where type = ? and user_id = ? and scene_id = ?
    #end

    #sql ("queryIsHideSystem")
    select count(scene_id) as number from admin_scene where scene_id in (
       #for(i : ids)
           #(for.index > 0 ? "," : "")#para(i)
       #end
    ) and is_system = 1
    ...
    #sql ("isHide")
    update admin_scene set is_hide = 1,sort = 0
    where scene_id in (
       #for(i : ids)
           #(for.index > 0 ? "," : "")#para(i)
       #end
    ...
    IF(deal_status = 0,(TO_DAYS(
      IFNULL((SELECT car.create_time FROM admin_record as car where car.types = 'crm_customer' and car.types_id = views.customer_id ORDER BY car.create_time DESC LIMIT 1),create_time))
      + CAST((SELECT value FROM admin_config WHERE name= 'customerPoolSettingFollowupDays') as SIGNED) - TO_DAYS(NOW())
    ),'') as pool_day,
    (select count(*) from crm_business as a where a.customer_id = views.customer_id) as business_count
    #end

    #sql("queryPutInPoolToday")
    select IFNULL(GROUP_CONCAT(customer_id),0)
    from crm_customer as a left join crm_owner_record as b on a.customer_id = b.type_id
    ...
    #end
    #sql ("queryBatchId")
      select batch_id from admin_fieldv where name=#para(name) and `value` #(connector) #(value)
      #if(batchList&&batchList.size()>0)
        and batch_id in ( #fori(batchList))
      #end
    #end
