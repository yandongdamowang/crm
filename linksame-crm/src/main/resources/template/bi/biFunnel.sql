#namespace("bi.funnel")
    #sql ("sellFunnel")
      ...
      ccb.type_id
      FROM crm_business as ccb
      LEFT JOIN crm_business_status as ccbs ON ccbs.status_id = ccb.status_id
      where
    ...
    #sql("sellFunnelList")
        ...
        left join admin_user c on a.owner_user_id = c.user_id
        left join crm_customer d on d.customer_id = a.customer_id
        left join crm_business_status e on e.status_id = a.status_id
        left join crm_business_type f on f.type_id = a.type_id
        where
