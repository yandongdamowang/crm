#namespace("work")
  ...
  #sql("workStatistics")
    select count(*) as allCount,
      ...
      from task where 1 = 1 and ishidden = 0
      #if(workId)
  ...
  #sql("queryRoleList")
  select role_id,role_name,remark from admin_role where role_type in (5,6) and status = 1
  #end

  #sql ("queryOwnerRoleList")
  select a.user_id,b.realname,a.role_id,c.role_name,b.img
  from work_user as a left join admin_user as b on a.user_id = b.user_id
  left join admin_role as c on a.role_id = c.role_id
  where a.work_id = ?
  #end
  #sql ("queryRealmByRoleId")
    select concat((select realm from `admin_menu` where menu_id = b.parent_id),':',b.realm) from `admin_role_menu` a left join `admin_menu` b on a.menu_id = b.menu_id where a.role_id = ? and b.menu_type = 3
  #end
  #sql("archList")
  select a.*,b.name as workName,(select count(*) from task_comment where type_id = a.task_id and type = 1) as commentCount,
         ...
         (select count(*) from task where pid = a.task_id) as childAllCount,
         (select count(*) from admin_file where batch_id = a.batch_id) as fileCount
  from task a left join work b on a.work_id = b.work_id where a.work_id = ? and a.is_archive = 1 and a.ishidden = 0
  #end
  #sql("queryTaskFileByWorkId")
    ...
    where a.batch_id in (select batch_id from `task` where work_id = #para(workId) and ishidden = 0)
  #end
  #sql("leave")
    update `task` set main_user_id = null where work_id =  #para(workId) and main_user_id = #para(userId);
  #end
  #sql("leave1")
    update `task` set owner_user_id = replace(owner_user_id,concat(',',#para(userId),','),',') where work_id = #para(workId) and  owner_user_id like concat(',',#para(userId),',');
  #end
  #sql("getTaskOwnerOnWork")
    select user_id,img,realname from `admin_user` where user_id in (select main_user_id from `task` where ishidden = 0 and work_id in (?))
  #end

  #sql ("queryProjectUser")
  select user_id,realname from admin_user as a left join admin_user_role as b on a.user_id = b.user_id
  left join admin_role_menu as c on b.role_id = c.role_id
  where d.menu_id = ?
  #end
#end
